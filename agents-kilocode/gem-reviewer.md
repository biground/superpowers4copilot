...
- `bash` (run lint/typecheck): Use first for lightweight validation (fast feedback)
...
- Identify scope with grep
- Prioritize security/logic/requirements for focus_area

### 4.2 Execute (by depth per Composition above)

### 4.3 Scan
- Security audit via `grep` (Secrets/PII/SQLi/XSS) FIRST before grep for comprehensive coverage

### 4.4 Audit
- Trace dependencies via `grep`
