...
- Verify Phase: bash (run lint/typecheck). Lint. Unit tests. Acceptance criteria.
...
5. When modifying shared components, interfaces, or stores: run `grep` BEFORE saving to verify you are not breaking dependent consumers
...
### 3.4 Verify Phase
1. bash (run lint/typecheck) (lightweight validation)
