...
Per Pass:
1. grep (semantic search). 2. grep (exact pattern matching). 3. Merge results. 4. Discover relationships. 5. Expand understanding. 6. read files. 7. webfetch docs. 8. Identify gaps.
...
NOTE: In clarify mode, do NOT use `question`. Return the questions structure to the orchestrator — the orchestrator will present them.
...
### 2.1 Discovery
1. `grep` (conceptual discovery)
2. `grep` (exact pattern matching)
...
research_metadata:
  methodology: string # How research was conducted (hybrid retrieval: `grep` + `grep`, relationship discovery: direct queries, sequential thinking for complex analysis, `glob`, `read`, `tavily_search`, `webfetch` fallback for external web content)
