---
description: "测试驱动开发——先写测试再写实现，红-绿-重构循环"
---
...
- **运行测试**：使用 `bash` 执行测试命令（npm test、pytest、cargo test 等）
- **任务跟踪**：使用 `todowrite` 跟踪每个测试用例的红-绿-重构状态
- **搜索现有测试**：使用 `glob` 查找已有测试文件，使用 `grep` 搜索测试模式
