...
- **运行测试**：使用 `bash` 执行测试命令确认全部通过
- **Git 操作**：使用 `bash` 执行 merge、push、pr create 等操作
- **呈现选项**：使用 `question` 提供 4 个结构化选项（本地合并 / 推送 PR / 保持现状 / 丢弃）
- **任务跟踪**：使用 `todowrite` 跟踪验证和清理步骤
