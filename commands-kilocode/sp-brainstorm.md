---
description: "通过结构化的苏格拉底式对话，对功能设计进行头脑风暴与细化"
---
...
- **提问**：使用 `question` 逐个提问（选择题优先），不要用纯文本列选项
- **任务跟踪**：使用 `todowrite` 跟踪检查清单（探索上下文 → 澄清 → 方案 → 设计 → spec → 自审 → 审阅 → 过渡到实施）
- **分派子代理**：如需分派子代理，使用 `task`
- **文件操作**：读文件用 `read`，搜索用 `grep` / `glob`
