---
description: "分派代码审查代理，获取结构化的改进建议"
---
...
- **获取变更信息**：使用 `bash` 运行 `git diff`、`git rev-parse` 获取 SHA
- **分派审查代理**：使用 `task` 分派 code-reviewer 子代理
- **读取审查模板**：使用 `read` 读取 skills/requesting-code-review/code-reviewer.md 模板
- **任务跟踪**：使用 `todowrite` 跟踪审查反馈的处理状态
