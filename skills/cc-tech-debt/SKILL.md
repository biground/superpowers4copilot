...
Use `grep_search` and `file_search` to search the codebase for debt indicators:
...
Use `vscode_askQuestions`: "May I write these findings to `docs/tech-debt-register.md`?"

If yes, use `replace_string_in_file` or `create_file` to update the register (append new entries, do not overwrite existing ones). Verdict: **COMPLETE** — scan findings written to register.
...
Use `vscode_askQuestions` to prompt for: description, category, affected files, estimated fix effort, impact if left unfixed.

Present the new entry to the user.

Use `vscode_askQuestions`: "May I append this entry to `docs/tech-debt-register.md`?"
...
Use `read_file` to read the debt register at `docs/tech-debt-register.md`.
...
Use `vscode_askQuestions`: "May I write the re-prioritized register back to `docs/tech-debt-register.md`?"
...
Use `read_file` to read the debt register. Generate summary statistics:
...
- Run `/cc-sprint-plan` to schedule high-priority debt items into the next sprint.
- Run `/cc-tech-debt report` at the start of each sprint to track debt trends over time.
