...
Use `read_file` to read `.claude/docs/technical-preferences.md` to determine:
...
Use `runSubagent` to spawn the `cc-security-engineer` agent. Pass:
...
The cc-security-engineer runs the audit across 6 categories (see Phase 3). Collect their full findings before proceeding.

---

## Phase 3: Audit Categories

The cc-security-engineer evaluates each of the following. Skip categories not applicable to the project scope.
...
Use `grep_search` for: `File.open`, `load`, `deserialize`, `JSON.parse`, `from_json`, `read_file` — check each for validation.
...
Use `grep_search` for: `recv`, `receive`, `PacketPeer`, `socket`, `NetworkedMultiplayerPeer`, `rpc`, `rpc_id` — check each call site for validation.
...
- Are numeric inputs bounds-checked before use?
- Are achievement/stat values checked before being written to any backend?

Use `grep_search` for: `get_input`, `Input.get_`, `input_map`, user-facing text fields — check validation.
...
Use `grep_search` for: `api_key`, `secret`, `password`, `token`, `private_key`, `DEBUG`, `print(` in release-facing code.

### Category 5: Cheat and Anti-Tamper Vectors
- Are gameplay-critical values stored only in memory, not in easily-editable files?
- Are any critical game progression flags validated server-side?
- Is there any protection against memory editing tools for multiplayer?
...
Use `file_search` for: `addons/`, `plugins/`, `third_party/`, `vendor/` — list all external dependencies.
...
**Audited by**: cc-security-engineer via /cc-security-audit
...
Run `/cc-security-audit` again after remediating any CRITICAL or HIGH findings.
...
Use `vscode_askQuestions`: "May I write the full security audit report to `production/security/security-audit-[date].md`?"
...
After remediating findings, re-run: `/cc-security-audit quick` to confirm CRITICAL/HIGH items are resolved before running `/cc-gate-check release`.

If CRITICAL findings exist:
> "CRITICAL security findings must be resolved before any public release. Do not proceed to `/cc-launch-checklist` until these are addressed."

If no CRITICAL/HIGH findings:
> "No blocking security findings. Report written to `production/security/`. Include this path when running `/cc-gate-check release`."
...
- **Accepted risk is a valid outcome** — some LOW findings are acceptable trade-offs; document the decision
