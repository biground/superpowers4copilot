...
Use `file_search`, `list_dir`, `grep_search`, and `read_file` to analyze project structure and content:
...
Based on scanned artifacts, determine stage. Use `read_file` to check `production/stage.txt` first —
if it exists, use its value (explicit override from `/cc-gate-check`). Otherwise,
...
| **Polish** | Explicit only (set by `/cc-gate-check` Production → Polish gate) |
| **Release** | Explicit only (set by `/cc-gate-check` Polish → Release gate) |

### 3. Collaborative Gap Identification

**DO NOT** just list missing files. Instead, **ask clarifying questions** using `vscode_askQuestions`:
...
- "I found a game concept but no systems index. Have you decomposed the concept into individual systems yet, or should we run `/cc-map-systems`?"
...
If user provided a role argument (e.g., `/cc-project-stage-detect programmer`):
...
```
- Options: "Yes — write the report", "No — I'll review first"

Wait for user approval before using `create_file` to write the file.
...
- **Concept exists but no systems index?** → `/cc-map-systems` to decompose into systems
- **Missing design docs?** → `/cc-reverse-document design src/[system]`
- **Missing architecture docs?** → `/cc-architecture-decision` or `/cc-reverse-document architecture`
- **Prototypes need documentation?** → `/cc-reverse-document concept prototypes/[name]`
- **No sprint plan?** → `/cc-sprint-plan`
- **Approaching milestone?** → `/cc-milestone-review`
