...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `/cc-launch-checklist`. Do not auto-invoke based on context matching.
...
- Use `read_file` to read `CLAUDE.md` for tech stack, target platforms, and team structure
- Use `file_search` and `read_file` to read the latest milestone in `production/milestones/`
- Use `file_search` and `read_file` to read any existing release checklist in `production/releases/`
- Use `read_file` to read `design/live-ops/content-calendar.md` if it exists

---

## Phase 3: Scan Codebase Health

- Use `grep_search` to count `TODO`, `FIXME`, `HACK` comments and their locations
- Use `grep_search` to check for any `console.log`, `print()`, or debug output left in production code
- Use `grep_search` to check for placeholder assets (search for `placeholder`, `temp_`, `WIP_`)
- Use `grep_search` to check for hardcoded test/dev values (localhost, test credentials, debug flags)
...
If not in dry-run mode, use `vscode_askQuestions`: "May I write this to `production/releases/launch-checklist-[date].md`?"

If yes, use `create_file` to write the file, creating directories as needed.

---

## Phase 6: Next Steps

- Run `/cc-gate-check` to get a formal PASS/CONCERNS/FAIL verdict before launch.
- Coordinate sign-offs via `/cc-team-release`.
