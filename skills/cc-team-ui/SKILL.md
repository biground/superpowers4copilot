---
name: cc-team-ui
description: "Orchestrate the UI team through the full UX pipeline: from UX spec authoring through visual design, implementation, review, and polish. Integrates with /cc-ux-design, /cc-ux-review, and studio UX templates."
---
...
## Team Composition
- **cc-ux-designer** — User flows, wireframes, accessibility, input handling
- **cc-ui-programmer** — UI framework, screens, widgets, data binding, implementation
- **cc-art-director** — Visual style, layout polish, consistency with art bible
...
- **cc-accessibility-specialist** — Audits accessibility compliance at Phase 4
...
- `cc-ux-designer` — User flows, wireframes, accessibility, input handling
- `cc-ui-programmer` — UI framework, screens, widgets, data binding
- `cc-art-director` — Visual style, layout polish, art bible consistency
- `cc-[UI engine specialist]` — Engine-specific UI pattern validation (e.g., cc-unity-ui-specialist, cc-ue-umg-specialist, cc-godot-specialist)
- `cc-accessibility-specialist` — Accessibility compliance audit
...
Before designing anything, use `read_file` to read and synthesize:
...
- (a) Run `/cc-ux-design patterns` first to establish the pattern library, then continue
- (b) Proceed without the pattern library — cc-ui-programmer will treat all patterns created as new and add each to a new `design/ux/interaction-patterns.md` at completion

Do NOT invent or assume patterns from the feature name or GDD alone. If the user chooses (b), explicitly instruct cc-ui-programmer in Phase 3 to treat all patterns as new and document them in `design/ux/interaction-patterns.md` when implementation is complete. Note the pattern library status (created / absent / updated) in the final summary report.

Summarize the context in a brief for the cc-ux-designer: what the player is doing, what they need, what constraints apply, and which existing patterns are relevant.

### Phase 1b: UX Spec Authoring

Invoke `/cc-ux-design [feature name]` skill OR delegate directly to cc-ux-designer via `runSubagent` to produce `design/ux/[feature-name].md` following the `ux-spec.md` template.

If designing the HUD, use the `hud-design.md` template instead of `ux-spec.md`.

> **Notes on special cases:**
> - For HUD design specifically, invoke `/cc-ux-design` with `argument: hud` (e.g., `/cc-ux-design hud`).
> - For the interaction pattern library, run `/cc-ux-design patterns` once at project start and update it whenever new patterns are introduced during later phases.

Output: `design/ux/[feature-name].md` with all required spec sections filled.

### Phase 1c: UX Review

After the spec is complete, invoke `/cc-ux-review design/ux/[feature-name].md`.

**Gate**: Do not proceed to Phase 2 until the verdict is APPROVED. If the verdict is NEEDS REVISION, the cc-ux-designer must address the flagged issues and re-run the review. The user may explicitly accept a NEEDS REVISION risk and proceed, but this must be a conscious decision — present the specific concerns via `vscode_askQuestions` before asking whether to proceed.

### Phase 2: Visual Design

Delegate to **cc-art-director** via `runSubagent`:
...
Before implementation begins, spawn the **engine UI specialist** via `runSubagent` (from `.claude/docs/technical-preferences.md` Engine Specialists → UI Specialist) to review the UX spec and visual design spec for engine-specific implementation guidance:
...
- Output: engine UI implementation notes to hand off to cc-ui-programmer before they begin

If no engine is configured, skip this step.

Delegate to **cc-ui-programmer** via `runSubagent`:
- Implement the UI following the UX spec and visual design spec
- **Use patterns from `design/ux/interaction-patterns.md`** — do not reinvent patterns that are already specified. If a pattern almost fits but needs modification, note the deviation and flag it for cc-ux-designer review.
...
- **cc-ux-designer**: Verify implementation matches wireframes and interaction spec. Test keyboard-only and gamepad-only navigation. Check accessibility features function correctly.
- **cc-art-director**: Verify visual consistency with art bible. Check at minimum and maximum supported resolutions.
- **cc-accessibility-specialist**: Verify compliance against the committed accessibility tier documented in `design/accessibility-requirements.md`. Flag any violations as blockers.
...
- `/cc-ux-design` — Author a new UX spec for a screen, flow, or HUD from scratch
- `/cc-ux-review` — Validate a completed UX spec before implementation
- `/cc-team-ui [feature]` — Full pipeline from concept through polish (calls `/cc-ux-design` and `/cc-ux-review` internally)
- `/cc-quick-design` — Small UI changes that don't need a full new UX spec
...
delegated to sub-agents and sub-skills (`/cc-ux-design`, cc-ui-programmer). Each enforces the
...
- Run `/cc-ux-review` on the final spec if not yet approved.
- Run `/cc-code-review` on the UI implementation before closing stories.
- Run `/cc-team-polish` if visual or audio polish pass is needed.
