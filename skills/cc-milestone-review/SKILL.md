...
2. Else use `read_file` to read `production/review-mode.txt` → use that value
...
Use `file_search` and `read_file` to read the milestone definition from `production/milestones/`. If the argument is `current`, use the most recently modified milestone file.

Use `file_search` and `read_file` to read all sprint reports for sprints within this milestone from `production/sprints/`.

---

## Phase 2: Scan Codebase Health

- Use `grep_search` to scan for `TODO`, `FIXME`, `HACK` markers that indicate incomplete work
- Use `read_file` to check the risk register at `production/risk-register/`
...
Before generating the Go/No-Go recommendation, use `runSubagent` to spawn the `cc-producer` agent via gate **PR-MILESTONE**.
...
Use `vscode_askQuestions`: "May I write this to `production/milestones/[milestone-name]-review.md`?"

If yes, use `create_file` to write the file, creating the directory if needed. Verdict: **COMPLETE** — milestone review saved.
...
- Run `/cc-gate-check` for a formal phase gate verdict if this milestone marks a development phase boundary.
- Run `/cc-sprint-plan` to adjust the next sprint based on the scope recommendations above.
