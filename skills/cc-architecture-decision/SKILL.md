...
(e.g., `/cc-architecture-decision retrofit docs/architecture/adr-0001-event-system.md`):

Enter **retrofit mode**:

1. Read the existing ADR file completely.
2. Identify which template sections are present by scanning headings:
   - `## Status` — **BLOCKING if missing**: `/cc-story-readiness` cannot check ADR acceptance
...
5. If yes:
   ...
   - Append each missing section to the ADR file using `replace_string_in_file`.
...
7. Suggest: "Run `/cc-architecture-review` to re-validate coverage now that this ADR
...
1. Use `file_search` to find `docs/engine-reference/*/VERSION.md`, then `read_file` to get:
...
4. Use `read_file` on `docs/engine-reference/[engine]/breaking-changes.md` — flag any
   changes in the relevant domain that post-date the LLM's training cutoff.

5. Use `read_file` on `docs/engine-reference/[engine]/deprecated-apis.md` — flag any APIs
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
   ...
   Run `/cc-setup-engine` first, or tell me which engine you are using."

---

## 1. Determine the next ADR number

Use `file_search` for `docs/architecture/adr-*.md` to find existing ADRs and determine the next number.
...
Use `read_file` on `docs/registry/architecture.yaml`. Extract entries relevant to this ADR's
domain and decision (use `grep_search` by system name, domain keyword, or state being touched).
...
**After assumptions are confirmed**, if the ADR involves schema or data design choices, use a separate multi-question `vscode_askQuestions` to ask each design question independently before drafting.
...
| **References Consulted** | [List engine-reference docs read] |
| **Post-Cutoff APIs Used** | [Any APIs from post-LLM-cutoff versions, or "None"] |
...
[The specific technical decision made, described in enough detail for implementation.]
...
|------------|-------------|--------------------------|
| [system-name].md | [specific rule/formula/constraint] | [how this decision satisfies it] |
...
4.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via `runSubagent` to validate the drafted ADR:
   - Read the project technical preferences file `Engine Specialists` section to get the primary specialist
   - If no engine is configured (`[TO BE CONFIGURED]`), skip this step
   - Spawn the primary specialist (e.g. `cc-godot-specialist`, `cc-unity-specialist`, or `cc-unreal-specialist`) with: the ADR's Engine Compatibility section, Decision section, Key Interfaces, and the engine reference docs path. Ask them to:
   ...
   - If the specialist identifies a **blocking issue**: revise the Decision and Engine Compatibility sections accordingly, then confirm the changes with the user before proceeding
...
- `solo` → skip. Note: "TD-ADR skipped — Solo mode." Proceed to Step 4.7.
- `lean` → skip (not a PHASE-GATE). Note: "TD-ADR skipped — Lean mode." Proceed to Step 4.7.
- `full` → spawn as normal.

4.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `cc-technical-director` via `runSubagent`:
...
4.7. **GDD Sync Check** — Before presenting the write approval, use `grep_search` across all GDDs
...
The GDD must be updated before or alongside writing this ADR.
```
...
If yes to any write option, use `create_file` to write the file, creating the directory if needed.
For option [A] with GDD update: also use `replace_string_in_file` on the GDD file(s) to use the new names.
...
**Registry append logic**: When writing to `docs/registry/architecture.yaml`, do NOT assume sections are empty. Before each edit:
1. Use `read_file` on the current state of `docs/registry/architecture.yaml`
2. Find the correct section (state_ownership, interfaces, forbidden_patterns, api_decisions)
3. Append the new entry AFTER the last existing entry in that section
...
Only proceed if the user selects yes. Never modify existing entries — if a stance is
...
1. Use `read_file` on `docs/registry/architecture.yaml` — check if any priority ADRs are still unwritten
2. Check if all prerequisite ADRs are now written. If yes, include a "Start writing GDDs" option.
3. List ALL remaining priority ADRs as individual options.

Widget format:
```
ADR-[NNNN] written and registry updated. What would you like to do next?
[1] Write [next-priority-adr-name] — [brief description]
[2] Write [another-priority-adr] — [brief description]
[N] Start writing GDDs — run /cc-design-system [first-undesigned-system]
[N+1] Stop here for this session
```

If there are no remaining priority ADRs and no undesigned GDD systems, offer only "Stop here" and suggest running `/cc-architecture-review` in a fresh session.

**Always include this fixed notice in the closing output:**

> To validate ADR coverage against your GDDs, open a **fresh session**
> and run `/cc-architecture-review`.
>
> **Never run `/cc-architecture-review` in the same session as `/cc-architecture-decision`.**
