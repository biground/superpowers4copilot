---
name: cc-team-live-ops
description: "Orchestrate the live-ops team for post-launch content planning: coordinates cc-live-ops-designer, cc-economy-designer, cc-analytics-engineer, cc-community-manager, cc-writer, and cc-narrative-director to design and plan a season, event, or live content update."
---

**Argument check:** If no season name or event description is provided, output:
> "Usage: `/cc-team-live-ops [season name or event description]` — Provide the name or description of the season or live event to plan."
...
## Team Composition
- **cc-live-ops-designer** — Season structure, event cadence, retention mechanics, battle pass
- **cc-economy-designer** — Live economy balance, store rotation, currency pricing, pity timers
- **cc-analytics-engineer** — Success metrics, A/B test design, event tracking, dashboard specs
- **cc-community-manager** — Player-facing announcements, event descriptions, seasonal messaging
- **cc-narrative-director** — Seasonal narrative theme, story arc, world event framing
- **cc-writer** — Event descriptions, reward item names, seasonal flavor text, announcement copy

## How to Delegate

Use `runSubagent` to spawn each team member as a subagent:
- `cc-live-ops-designer` — Season/event structure and retention mechanics
- `cc-economy-designer` — Live economy balance and reward pricing
- `cc-analytics-engineer` — Success metrics, A/B tests, event instrumentation
- `cc-community-manager` — Player-facing communication and messaging
- `cc-narrative-director` — Seasonal theme and narrative framing
- `cc-writer` — All player-facing text: event descriptions, item names, copy
...
Delegate to **cc-live-ops-designer** via `runSubagent`:
...
Delegate to **cc-economy-designer** via `runSubagent`:
...
Delegate to **cc-analytics-engineer** via `runSubagent`:
...
- **cc-narrative-director** (if needed): Write any in-game narrative text (cutscene scripts, NPC dialogue, world event descriptions) for the season
- **cc-writer**: Write all player-facing text — event names, reward item descriptions, challenge objective text, seasonal flavor text
- Both should read the narrative framing doc from Phase 2

### Phase 6: Player Communication Plan
Delegate to **cc-community-manager** via `runSubagent`:
...
- Run `/cc-design-review` on the season design document for consistency validation.
- Run `/cc-sprint-plan` to schedule content creation work for the season.
- Run `/cc-team-release` when the season content is ready to deploy.
