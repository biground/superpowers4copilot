---
name: cc-team-polish
description: "Orchestrate the polish team: coordinates cc-performance-analyst, cc-technical-artist, cc-sound-designer, and cc-qa-tester to optimize, polish, and harden a feature or area for release quality."
---

If no argument is provided, output usage guidance and exit without spawning any agents:
> Usage: `/cc-team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`).
...
## Team Composition
- **cc-performance-analyst** — Profiling, optimization, memory analysis, frame budget
- **cc-engine-programmer** — Engine-level bottlenecks: rendering pipeline, memory, resource loading (invoke when cc-performance-analyst identifies low-level root causes)
- **cc-technical-artist** — VFX polish, shader optimization, visual quality
- **cc-sound-designer** — Audio polish, mixing, ambient layers, feedback sounds
- **cc-tools-programmer** — Content pipeline tool verification, editor tool stability, automation fixes (invoke when content authoring tools are involved in the polished area)
- **cc-qa-tester** — Edge case testing, regression testing, soak testing

## How to Delegate

Use `runSubagent` to spawn each team member as a subagent:
- `cc-performance-analyst` — Profiling, optimization, memory analysis
- `cc-engine-programmer` — Engine-level fixes for rendering, memory, resource loading
- `cc-technical-artist` — VFX polish, shader optimization, visual quality
- `cc-sound-designer` — Audio polish, mixing, ambient layers
- `cc-tools-programmer` — Content pipeline and editor tool verification
- `cc-qa-tester` — Edge case testing, regression testing, soak testing
...
### Phase 1: Assessment
Delegate to **cc-performance-analyst** via `runSubagent`:
- Profile the target feature/area using `/cc-perf-profile`
...
Delegate to **cc-performance-analyst** via `runSubagent` (with relevant programmers as needed):
...
If Phase 1 identified engine-level root causes (rendering pipeline, resource loading, memory allocator), delegate those fixes to **cc-engine-programmer** via `runSubagent` in parallel:
...
Delegate to **cc-technical-artist** via `runSubagent`:
...
Delegate to **cc-sound-designer** via `runSubagent`:
...
- If READY FOR RELEASE: run `/cc-release-checklist` for the final pre-release validation.
- If NEEDS MORE WORK: schedule remaining issues in `/cc-sprint-plan update` and re-run `/cc-team-polish` after fixes.
- Run `/cc-gate-check` for a formal phase gate verdict before handing off to release.
