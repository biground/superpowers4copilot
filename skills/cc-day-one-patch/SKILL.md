...
- Use `file_search` for `production/qa/bugs/*.md` — load all bugs with Status: Open or Fixed — Pending Verification
...
- Options: `Approve this scope` / `Adjust — I want to add or remove items` / `No day-one patch needed`

If no patch needed: output "No day-one patch required. Proceed to `/cc-launch-checklist`." Stop.

### Step 2c — Check total scope

Sum estimated effort. If total exceeds 1 day of work:
> "Warning: Patch scope is [N hours] — this exceeds a safe day-one window. Consider deferring lower-priority items to patch 1.1. A bloated day-one patch introduces more risk than it removes."
...
Use `runSubagent` to spawn the `cc-release-manager` agent. Ask them to produce a rollback plan covering:
...
Present the rollback plan. Ask via `vscode_askQuestions`: "May I write this rollback plan to `production/releases/rollback-plan-[version].md`?"
...
1. Use `runSubagent` to spawn the `cc-lead-programmer` agent with:
...
2. The cc-lead-programmer implements and runs targeted tests.

3. Use `runSubagent` to spawn the `cc-qa-tester` agent to verify: does the bug reproduce after the fix?
...
This is a lightweight QA pass — not a full `/cc-team-qa`. The patch is already QA-approved from the release gate; we are only re-verifying the changed areas.

Use `runSubagent` to spawn the `cc-qa-lead` agent with:
- List of all changed files
- List of bugs fixed (with verification status from Phase 4)
- The smoke check scope for the affected systems

Ask cc-qa-lead to determine: **Is a targeted smoke check sufficient, or do any fixes touch systems that require a broader regression?**

Run the required QA scope:
- **Targeted smoke check** — run `/cc-smoke-check [affected-systems]`
...
- [ ] cc-lead-programmer: all fixes reviewed
- [ ] cc-qa-lead: QA gate PASS confirmed
- [ ] cc-producer: deployment timing approved
- [ ] cc-release-manager: platform submission confirmed

---

## Player-Facing Patch Notes

[Draft for cc-community-manager to review before publishing]

[list player-facing changes in plain language]
```

Use `vscode_askQuestions`: "May I write this patch record to `production/releases/day-one-patch-[version].md`?"

If approved, use `create_file` to write the file.
...
1. Run `/cc-patch-notes` to generate the player-facing version of the patch notes
2. Run `/cc-bug-report verify [BUG-ID]` for each fixed bug after the patch is live
3. Schedule a post-launch review 48–72 hours after launch using `/cc-retrospective launch`

**If any S1 bugs remain open after the patch:**
> "Warning: S1 bugs remain open and were not patched. These are accepted risks. Document them in the rollback plan trigger conditions — if they occur at scale, rollback may be preferable to a follow-up patch."
...
- **Player communication is part of the patch** — `/cc-patch-notes` is a required output, not optional
