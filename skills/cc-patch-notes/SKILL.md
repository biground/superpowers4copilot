...
If no version is provided, use `vscode_askQuestions` to ask the user before proceeding.

---

## Phase 2: Gather Change Data

- Use `read_file` to read the internal changelog at `production/releases/[version]/changelog.md` if it exists
- Also check `docs/CHANGELOG.md` for the relevant version entry
- Run `git log` via `run_in_terminal` between the previous release tag and current tag/HEAD as a fallback
- Use `file_search` and `read_file` to read sprint retrospectives in `production/sprints/` for context
- Use `read_file` to read any balance change documents in `design/balance/`
- Read bug fix records from QA if available

**If no changelog data is available**:

> "No changelog data found for [version]. Run `/cc-changelog [version]` first to generate the
> internal changelog, then re-run `/cc-patch-notes [version]`."
...
1. Use `read_file` to check `.claude/docs/technical-preferences.md` for any "tone", "voice", or "style" fields.
2. Use `file_search` to check `docs/PATCH-NOTES-STYLE.md` if it exists.
3. Use `file_search` to check `design/community/tone-guide.md` if it exists.
4. If any source contains tone/voice/style instructions, extract them and apply to the generated notes.
5. If no tone guidance is found, default to: player-friendly, non-technical language; enthusiastic but not hyperbolic; focus on what the player experiences.

**Template detection** — check whether a patch notes template exists:

1. Use `file_search` for `docs/patch-notes-template.md` and `.claude/docs/templates/patch-notes-template.md`.
2. If found, read it and use it as the output structure instead of the built-in style templates.
...
- Tone matches the game's voice

---

## Phase 6: Save Patch Notes

Present the completed patch notes to the user along with: a count of changes by category, and any internal changes that were excluded.

Use `vscode_askQuestions`: "May I write these patch notes to `docs/patch-notes/[version].md`?"

If yes, use `create_file` to write the file to `docs/patch-notes/[version].md`, creating the directory
...
- Run `/cc-release-checklist` to verify all other release gates are met before publishing.
- Share the patch notes draft with the cc-community-manager for tone review before posting publicly.
