...
Use `read_file` to check for existing performance targets in design docs or CLAUDE.md:
...
Use `grep_search`, `file_search`, and `read_file` to analyze the following:
...
Present significant-effort items and use `vscode_askQuestions` to ask the user to choose for each:

- **Implement the optimization** (proceed with fix now or schedule it)
- **Reduce feature scope** (run `/cc-scope-check [feature]` to analyze trade-offs)
- **Accept the performance hit and defer to Polish phase** (log as known issue)
- **Escalate to technical-director for an architectural decision** (run `/cc-architecture-decision`)

If multiple items are deferred to Polish, record them under `### Deferred to Polish`.
...
- If bottlenecks require architectural change: run `/cc-architecture-decision`.
- If scope reduction is needed: run `/cc-scope-check [feature]`.
- To schedule optimizations: run `/cc-sprint-plan update`.
