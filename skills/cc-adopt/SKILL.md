---
name: cc-adopt
description: "Brownfield onboarding — audits existing project artifacts for template format compliance (not just existence), classifies gaps by impact, and produces a numbered migration plan. Run this when joining an in-progress project or upgrading from an older template version. Distinct from /cc-project-stage-detect (which checks what exists) — this checks whether what exists will actually work with the template's skills."
---
...
**This is not `/cc-project-stage-detect`.**
`/cc-project-stage-detect` answers: *what exists?*
`/cc-adopt` answers: *will what exists actually work with the template's skills?*
...
### Existence check
- `production/stage.txt` — if present, use `read_file` to read it (authoritative phase)
- `design/gdd/game-concept.md` — concept exists?
- `design/gdd/systems-index.md` — systems index exists?
- Count GDD files: use `file_search` for `design/gdd/*.md` (excluding game-concept.md and systems-index.md)
- Count ADR files: use `file_search` for `docs/architecture/adr-*.md`
- Count story files: use `file_search` for `production/epics/**/*.md` (excluding EPIC.md)
- `.claude/docs/technical-preferences.md` — engine configured?
- `docs/engine-reference/` — engine reference docs present?
- Use `file_search` for `docs/adoption-plan-*.md` — note the filename of the most recent prior plan if any exist

### Infer phase (if no stage.txt)
Use the same heuristic as `/cc-project-stage-detect`:
...
- Nothing → Fresh (not a brownfield project — suggest `/cc-start`)

If the project appears fresh (no artifacts at all), use `vscode_askQuestions`:
- "This looks like a fresh project — no existing artifacts found. `/cc-adopt` is for
  projects with work to migrate. What would you like to do?"
  - "Run `/cc-start` — begin guided first-time onboarding"
...
For each GDD file found, use `read_file` to check for the 8 required sections by scanning headings:
...
For each ADR file found, use `read_file` to check for these critical sections:

| Section | Impact if missing |
|---|---|
| `## Status` | **BLOCKING** — `/cc-story-readiness` ADR status check silently passes everything |
| `## ADR Dependencies` | HIGH — dependency ordering in `/cc-architecture-review` breaks |
...
1. **Parenthetical status values** — use `grep_search` for any Status cell containing
   parentheses: `"Needs Revision ("`, `"In Progress ("`, etc.
   These break exact-string matching in `/cc-gate-check`, `/cc-create-stories`,
   and `/cc-architecture-review`. **BLOCKING.**
...
| Sprint status | `production/sprint-status.yaml` | MEDIUM — `/cc-sprint-status` falls back to markdown |
...
Use `read_file` to read `.claude/docs/technical-preferences.md`. Check each field for `[TO BE CONFIGURED]`:
...
`/cc-architecture-decision retrofit docs/architecture/adr-[NNNN]-[slug].md`
...
`/cc-design-system retrofit design/gdd/[filename].md`
...
2. Run `/cc-architecture-review` → bootstraps `tr-registry.yaml`
3. Run `/cc-create-control-manifest` → creates manifest with version stamp
4. Run `/cc-sprint-plan update` → creates `sprint-status.yaml`
5. Run `/cc-gate-check [phase]` → writes `stage.txt` authoritatively
...
Gap counts:
  ...
  HIGH:     [N] — unsafe to run /cc-create-stories or /cc-story-readiness
...
If approved, use `create_file` to write `docs/adoption-plan-[date].md` with this structure:
...
Re-run `/cc-adopt` anytime to check remaining gaps.
...
Run `/cc-architecture-review` — even if ADRs already exist, this run bootstraps
...
Run `/cc-gate-check [current-phase]`
**Time**: 5 min
...
auto-pass when the fields are absent. They won't benefit from TR-ID staleness
tracking or manifest version checks until they're regenerated.
Do not regenerate stories that are already in progress.
```
