---
name: cc-team-qa
description: "Orchestrate the QA team through a full testing cycle. Coordinates cc-qa-lead (strategy + test plan) and cc-qa-tester (test case writing + bug reporting) to produce a complete QA package for a sprint or feature. Covers: test plan generation, test case writing, smoke check gate, manual QA execution, and sign-off report."
---
...
- **cc-qa-lead** — QA strategy, test plan generation, story classification, sign-off report
- **cc-qa-tester** — Test case writing, bug report writing, manual QA documentation

## How to Delegate

Use `runSubagent` to spawn each team member as a subagent:
- `cc-qa-lead` — Strategy, planning, classification, sign-off
- `cc-qa-tester` — Test case writing and bug report writing

Always provide full context in each agent's prompt (story file paths, QA plan path, scope constraints). Launch independent cc-qa-tester tasks in parallel where possible (e.g., multiple stories in Phase 5 can be scaffolded simultaneously).
...
1. Detect the current sprint or feature scope from the argument:
   - If argument is a sprint identifier (e.g., `sprint-03`): use `file_search` and `read_file` to read all story files in `production/sprints/[sprint]/`
   - If argument is `feature: [system-name]`: use `file_search` to glob story files tagged for that system
   - If no argument: use `read_file` on `production/session-state/active.md` and `production/sprint-status.yaml` (if present) to infer the active sprint

2. Use `read_file` on `production/stage.txt` to confirm the current project phase.
...
### Phase 2: QA Strategy (cc-qa-lead)

Spawn `cc-qa-lead` via `runSubagent` to review all in-scope stories and produce a QA strategy.

Prompt the cc-qa-lead to:
...
If the smoke check result is **FAIL**, the cc-qa-lead must list the failures prominently. QA cannot proceed past the strategy phase with a failed smoke check.

Present the cc-qa-lead's full strategy to the user, then use `vscode_askQuestions`:

Options:
- "Looks good — proceed to test plan"
- "Adjust story types before proceeding"
- "Skip blocked stories and proceed with the rest"
- "Smoke check failed — fix issues and re-run /cc-team-qa"
- "Cancel — resolve blockers first"

If smoke check **FAIL**: do not proceed to Phase 3. Surface the failures and stop. The user must fix them and re-run `/cc-team-qa`.
...
### Phase 4: Test Case Writing (cc-qa-tester)
...
Spawn `cc-qa-tester` via `runSubagent` for each story (run in parallel where possible), providing:
- The story file path
...
After each FAIL result: use `vscode_askQuestions` to collect the failure description, then spawn `cc-qa-tester` via `runSubagent` to write a formal bug report in `production/qa/bugs/`.
...
Spawn `cc-qa-lead` via `runSubagent` to produce the sign-off report using all results from Phases 4–5.
...
- APPROVED: "Build is ready for the next phase. Run `/cc-gate-check` to validate advancement."
- APPROVED WITH CONDITIONS: "Resolve conditions before advancing. S3/S4 bugs may be deferred to polish."
- NOT APPROVED: "Resolve S1/S2 bugs and re-run `/cc-team-qa` or targeted manual QA before advancing."
